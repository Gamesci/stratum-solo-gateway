...
export function merkleBranchesForCoinbaseAt0(txidsBE: string[]): string[] {
  if (txidsBE.length === 0) return [];
  
  const leaves: Buffer[] = [Buffer.alloc(32, 0)];
  for (const txid of txidsBE) {
    leaves.push(toLE(txid));
  }
  ...
  while (layer.length > 1) {
    ...
    for (let i = 0; i < layer.length; i += 2) {
      ...
      const parent = dsha256(Buffer.concat([left, right]));
...
export function addressToScriptPubKey(addr: string): ScriptPubKey {
  if (addr.startsWith('bc1') || addr.startsWith('tb1') || addr.startsWith('bcrt1')) {
    try {
      ...
      throw new Error('Unsupported bech32 address format');
    } catch (e) {
  ...
  try {
    ...
    throw new Error(`Invalid base58 address: ${addr}`);
  }
}
...
function base58checkDecode(addr: string): Buffer {
  ...
  if (!hash.slice(0, 4).equals(cksum)) {
